...
import Batch, { IBatch } from "./models/batch_model";
import Student, { IStudent } from "./models/student_model";
import Category, { ICategory } from "./models/category_model";
import Item, { IItem } from "./models/items_model";
...
const importData = async (): Promise<void> => {
  try {
    ...
    for (let i = 0; i < students.length; i++) {
      const studentData = {
        ...students[i],
        batchId: createdBatches[i % createdBatches.length]
    ...
    const itemsWithStudents = items.map((item, index) => {
      ...
      const category = createdCategories[itemCategoryMap[index]]
        ._id as Types.ObjectId;
